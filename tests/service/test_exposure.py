...
class TestCumulativeReservation:
    ...
    def test_a_single_order_within_the_cap_is_admitted(self, tracker) -> None:
        assert tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0) is True
        assert tracker.used("bot-a") == pytest.approx(60.0)

    def test_a_second_order_breaching_the_cap_is_refused(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)

        assert tracker.reserve("bot-a", "c2", 60.0, per_bot_cap=100.0, global_cap=1_000.0) is False
    ...
    def test_orders_up_to_the_cap_are_admitted(self, tracker) -> None:
        assert tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0) is True
        assert tracker.reserve("bot-a", "c2", 40.0, per_bot_cap=100.0, global_cap=1_000.0) is True
    ...
    def test_re_reserving_at_a_higher_figure_replaces_the_old_one(
        ...
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reserve("bot-a", "c1", 80.0, per_bot_cap=100.0, global_cap=1_000.0)
...
class TestPerBotIsolation:
    def test_bots_have_separate_caps(self, tracker) -> None:
        assert tracker.reserve("bot-a", "c1", 90.0, per_bot_cap=100.0, global_cap=1_000.0) is True
        assert tracker.reserve("bot-b", "c2", 90.0, per_bot_cap=100.0, global_cap=1_000.0) is True

    def test_exposure_is_reported_per_bot(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 90.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reserve("bot-b", "c2", 30.0, per_bot_cap=100.0, global_cap=1_000.0)
    ...
    def test_the_global_cap_binds_across_bots(self) -> None:
        ...
        assert tracker.reserve(
            "bot-a", "c1", 90.0, per_bot_cap=100.0, global_cap=150.0
        ) is True

        # Within its own cap, but the shared budget is nearly spent.
        assert tracker.reserve(
            "bot-b", "c2", 90.0, per_bot_cap=100.0, global_cap=150.0
        ) is False
...
class TestSettlement:
    ...
    def test_releasing_frees_the_cap_for_another_order(self, tracker) -> None:
        ...
        assert tracker.reserve("bot-a", "c2", 90.0, per_bot_cap=100.0, global_cap=1_000.0) is True
...
class TestReleaseAll:
    def test_releasing_a_bot_clears_its_exposure(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reserve("bot-a", "c2", 30.0, per_bot_cap=100.0, global_cap=1_000.0)
...
class TestInvalidInput:
    ...
    def test_an_unusable_notional_is_refused(self, tracker, notional) -> None:
        assert tracker.reserve("bot-a", "c1", notional, per_bot_cap=100.0, global_cap=1_000.0) is False
        assert tracker.used("bot-a") == 0.0

    def test_a_negative_notional_is_refused(self, tracker) -> None:
        assert tracker.reserve("bot-a", "c1", -5.0, per_bot_cap=100.0, global_cap=1_000.0) is False

    def test_a_zero_cap_admits_nothing(self, tracker) -> None:
        assert tracker.reserve("bot-a", "c1", 1.0, per_bot_cap=0.0, global_cap=1_000.0) is False
...
class TestConcurrency:
    ...
    def test_concurrent_reservations_cannot_exceed_the_global_cap(self) -> None:
        ...
        def submit(index: int) -> None:
            ...
            ok = tracker.reserve(
                f"bot-{index}", f"c{index}", 20.0,
                per_bot_cap=100.0, global_cap=100.0,
            )
            with lock:
                admitted.append(ok)
    ...
    def test_concurrent_reservations_on_one_bot_respect_its_cap(self) -> None:
        ...
        def submit(index: int) -> None:
            start.wait()
            ok = tracker.reserve(
                "bot-a", f"c{index}", 25.0, per_bot_cap=100.0, global_cap=10_000.0,
            )
            with lock:
                admitted.append(ok)
...
class TestReduction:
    """Reduce-only fills shrink a position, so they release budget."""

    def test_reducing_lowers_the_bots_exposure(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reduce_bot("bot-a", 20.0)

        assert tracker.used("bot-a") == pytest.approx(40.0)

    def test_reducing_below_zero_floors_at_zero(self, tracker) -> None:
        # A venue reporting more reduced than we thought was held must not
        # produce negative exposure, which would hand out free budget.
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reduce_bot("bot-a", 500.0)

        assert tracker.used("bot-a") == 0.0

    def test_reduction_is_spread_across_the_bots_orders(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=200.0, global_cap=1_000.0)
        tracker.reserve("bot-a", "c2", 40.0, per_bot_cap=200.0, global_cap=1_000.0)
        tracker.reduce_bot("bot-a", 50.0)

        assert tracker.used("bot-a") == pytest.approx(50.0)

    def test_reducing_leaves_other_bots_untouched(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reserve("bot-b", "c2", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reduce_bot("bot-a", 60.0)

        assert tracker.used("bot-b") == pytest.approx(60.0)

    def test_reducing_an_unknown_bot_is_harmless(self, tracker) -> None:
        tracker.reduce_bot("never-traded", 10.0)

        assert tracker.total() == 0.0

    def test_an_unusable_reduction_is_ignored(self, tracker) -> None:
        tracker.reserve("bot-a", "c1", 60.0, per_bot_cap=100.0, global_cap=1_000.0)
        tracker.reduce_bot("bot-a", float("nan"))
